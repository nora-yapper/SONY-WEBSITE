...
export function FindYourRegionSection() {
  const regions = [
    ...
    {
      name: "North Country",
      counties: "Clinton, Essex, Franklin, Fulton, Hamilton, Montgomery, Saratoga (Northern), Schoharie, Warren",
      color: "bg-black",
      textColor: "text-white",
      href: "/regions",
    },
  ...
  return (
    <section className="relative py-24 bg-gray-50">
      <div className="container mx-auto px-4">
        <h2 className="text-4xl md:text-5xl font-bold text-black mb-12 uppercase tracking-tight">Find Your Region</h2>
          ...
          {regions.map((region) => (
            <Link key={region.name} href={region.href}>
              ...
              >
                ...
                <Button
                  variant="outline"
                  className={`rounded-full ${region.textColor === "text-white" ? "border-white text-white hover:bg-white hover:text-black" : "border-black text-black hover:bg-black hover:text-white"}`}
                >
                  Learn More
                </Button>
              </Card>
